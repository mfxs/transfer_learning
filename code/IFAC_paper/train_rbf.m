function [alpha_best,sv_best,bias_best,C_best,bandwidth_best,hist]=train_rbf(x_train,y_train,x_validation,y_validation,C,bandwidth)
% RBF核模型训练
...
%         采用一对多的方式进行训练
        ...
        ktrain=[(1:size(x_train,1))' rbf_kernel(x_train,x_train,sigma)];
...
%         记录不同参数下训练和验证的准确率
        ...
        [~,temp]=pred(x_train,y_train,alpha,bias,sv,'rbf',sigma);
        disp('Predict on validation set.');
        [~,accuracy]=pred(x_validation,y_validation,alpha,bias,sv,'rbf',sigma);

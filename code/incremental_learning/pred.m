function [y_pred,accuracy]=pred(x_test,y_test,alpha,bias,supportvector,s,kernel_weight,InfoKernel)
% 测试
...
% 计算每一个二分类器的预测结果
for i=1:length(bias)
    disp(strcat('classifier',num2str(i)));
    switch s 
        case 'respective'
        ...
        case 'rbf'
            Kt=kernel_rbf(x_test,supportvector{i});
        case 'average'
            Kt=kernel(x_test,supportvector{i});
        case 'ovo'
            Kt=mklkernel(x_test,InfoKernel,ones(size(kernel_weight(i,:))),supportvector{i},kernel_weight(i,:));
    end
